...
@section('content_header')
    <h1 class="card-header">{{ __('admin.edit_admin') }} : {{ $admin->name }}</h1>
@stop
    ...
    <div class="card card-primary">
        <form action="{{ route('dashboard.admins.update', $admin->id) }}" method="POST">
                ...
                <div class="form-group">
                    <div class="row">
                        <x-adminlte-input name="name" label="{{ __('admin.name') }}" type="text"
                            ...
                            value="{{ $admin->name }}" />
                        <x-adminlte-input name="admin_id" type="hidden" value="{{ $admin->id }}" />
                    ...
                    <div class="row">
                        ...
                        <select name="type" class="form-control">
                            ...
                            <option value="super-admin" @selected($admin->type === 'super-admin')>{{ __('admin.super_admin') }}</option>
                            <option value="content-management" @selected($admin->type === 'content-management')>
                                {{ __('admin.content_management') }}</option>
                        </select>
            ...
            <div class="card-footer">
                <x-adminlte-button type="submit" theme="primary" label="{{ __('admin.update') }}" />
            </div>
        </form>
    </div>
@stop
